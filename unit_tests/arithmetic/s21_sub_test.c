...
Suite *s21_sub_suite() {
  ...
  tcase_add_test(tc_sub, low_great);
...
static void common_error_test(int m1[3], int p1, int m2[3], int p2) {
  s21_decimal vals[] = {s21_ints_to_decimal(m1, p1, 0),
                        s21_ints_to_decimal(m2, p2, 1)};
  for (int j = 0; j < 2; j++) {
    s21_decimal tmp;
    int out = s21_sub(vals[j], vals[(j + 1) % 2], &tmp);
    if (j) {
      ck_assert(out == 2);
    } else {
      ck_assert(out == 1);
...
static void common_sub_test(int m1[3], int p1, int m2[3], int p2, int mr[3],
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (!s21_is_zero(&result)) {
        ck_assert(s21_get_power(&result) == pr);
        ck_assert(s21_get_sign(&result) == (i ^ ((j + sr) % 2)));
      }
    }
  }
}