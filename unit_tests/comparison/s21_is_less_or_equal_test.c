...
START_TEST(less) {
  ...
  ck_assert(s21_is_less_or_equal(val1, val2));
}
END_TEST
...
START_TEST(equal) {
  ...
  ck_assert(s21_is_less_or_equal(val1, val2));
  ck_assert(s21_is_less_or_equal(val2, val1));
  s21_set_sign(&val1, 1);
  s21_set_sign(&val2, 1);
  ck_assert(s21_is_less_or_equal(val1, val2));
  ck_assert(s21_is_less_or_equal(val2, val1));
...
Suite *s21_is_less_or_equal_suite() {
  ...
  tcase_add_loop_test(tc_true, less, 0, LENGTH(t_less_p1));
  tcase_add_loop_test(tc_true, equal, 0, LENGTH(t_equal_p1));
  tcase_add_test(tc_true, equal_zero);
  suite_add_tcase(s, tc_true);

  TCase *tc_false = tcase_create("FALSE");
  tcase_add_loop_test(tc_false, greater, 0, LENGTH(t_less_p1));
